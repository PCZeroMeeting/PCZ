...
class Forum_model extends CI_Model {
	...
	public function create_topic($forum_id, $title, $content, $user_id) {
		...
		if ($this->db->insert('forumtopics', $data)) {
	...
	public function create_post($topic_id, $user_id, $content) {
		...
		if ($this->db->insert('forumposts', $data)) {
			
			$data = array('updated_at' => date('Y-m-j H:i:s'));
			$this->db->where('id', $topic_id);
			return $this->db->update('forumtopics', $data);
	...
	public function count_forum_posts($forum_id) {
		...
		$this->db->select('forumposts.id');
		$this->db->from('forumposts');
		$this->db->join('forumtopics', 'forumposts.topic_id = forumtopics.id');
		$this->db->where('forumtopics.forum_id', $forum_id);
		$this->db->group_by('forumposts.id');
